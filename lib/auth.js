...
import { isCounselorApproved } from "./permissions.js";
import { sanitizeAuditDetails } from "./audit-log.js";

// Re-exported so existing imports (`validatePasswordStrength` from lib/auth)
// keep working. The implementation lives in password-policy.js, which has no
// Prisma dependency and can therefore also be used by client-side forms.
...
export async function validateSession(token) {
  ...
  // Defence in depth: a counselor whose registration has not been approved yet
  // (or was rejected) can never hold a usable session, even if an approval is
  // revoked while a session exists.
  if (session.user.role === "COUNSELOR" && !isCounselorApproved(session.user)) {
    await invalidateSession(token);
    return null;
  }
  ...
  try {
    await prisma.auditLog.create({
      data: {
        userId,
        action,
        // Secrets must never reach the audit trail, so details are stripped of
        // password/token-like keys before they are serialized.
        details: JSON.stringify(sanitizeAuditDetails(details)),
        ipAddress,
      },
    });
  } catch {
